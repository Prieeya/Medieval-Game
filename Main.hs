...
-- Medieval Siege Simulator - Enhanced Edition
-- Advanced AI with Learning & Polished UI
...
spawnWave state =
  let newWave = wave state + 1
      ...
      -- Determine enemy composition based on wave and AI learning
      (numBasic, numFast, numTank, numBoss) = calculateEnemyComposition newWave adaptLevel (aiMemory state)
      
      -- Create spawn positions with some randomization
...
calculateEnemyComposition waveNum adaptLvl aiMem =
  let baseBasic = 3 + waveNum
      ...
      -- Adapt based on previous losses
      ...
      -- If fast enemies survive better, send more
...
updateEnemyState dt state enemy =
  let healthPercent = fromIntegral (enemyHealth enemy) / fromIntegral (enemyMaxHealth enemy)
      nearbyTowers = filter (\t -> distance (enemyPos enemy) (towerPos t) < towerRange t * 1.2) (towers state)
     ...
     then case findTarget tower enms of
            Just target ->
              let dmg = towerDamage (towerType tower) + (towerLevel tower - 1) * 5
                  ...
                  flashPart = Particle (towerPos tower) (0, 0) yellow 0.2 8
...
findTarget tower enms =
  let inRange = filter (\e -> distance (towerPos tower) (enemyPos e) <= towerRange tower && enemyState e /= Retreating) enms
      -- Prioritize based on tower type
      prioritized = case towerType tower of
        Arrow -> sortBy (comparing (negate . enemyHealth)) inRange  -- Target weak
        Cannon -> sortBy (comparing (negate . enemyMaxHealth)) inRange  -- Target tanks
        Ice -> sortBy (comparing (distance (towerPos tower) . enemyPos)) inRange  -- Target close
        Lightning -> sortBy (comparing (negate . enemySpeed)) inRange  -- Target fast
...
updateParticle dt part =
  ...
  in if newLife <= 0
     then Nothing
              ...
              newVel = (vx * 0.98, vy * 0.98 - 50 * dt)  -- Gravity
  ...
  let projectileHits = [(projTarget p, projDamage p, projType p, projPos p) | p <- projectiles state]
      ...
      (damagedCastle, filteredEnemies, dmgParticles) = foldl (\(c, es, ps) e ->
        ...
        then let dmg = if castleShield c > 0 then 0 else 2
                 ...
                 hitParts = [Particle (castlePos c) (20 * cos a, 20 * sin a) red 0.8 5 | a <- [0, pi/4..2*pi]]
      ...
      allParticles = deathParticles ++ dmgParticles ++ particles state
...
createDeathParticles :: Enemy -> [Particle]
createDeathParticles enemy =
  let (x, y) = enemyPos enemy
      colors = [red, orange, yellow]
      angles = [0, pi/6..2*pi]
  in [Particle (x, y) (60 * cos a, 60 * sin a) (colors !! (floor a `mod` 3)) 0.6 (4 + fromIntegral (floor a `mod` 3)) | a <- angles]

updateAIMemory :: GameState -> GameState
updateAIMemory state =
  let currentMem = aiMemory state
      -- Update tower threat map
      ...
      -- Increase adaptation level every 3 waves
  ...
  case selectedTool state of
    ...
    Just ttype ->
      ...
      in if gold (resources state) >= cost && not tooClose && not nearCastle
         then let newTower = Tower pos ttype 0 0 1 (baseTowerRange ttype)
      ...
      in case nearbyTowers of
           ...
           (tower:_) ->
             ...
             in if gold (resources state) >= upgradeCost && towerLevel tower < 3
                then let upgradedTower = tower { towerLevel = towerLevel tower + 1, towerRange = towerRange tower * 1.15 }
...
render state =
  pictures
    ...
    , pictures (map (drawTower state) (towers state))
...
drawTower :: GameState -> Tower -> Picture
drawTower state t =
  let (x, y) = towerPos t
      col = case towerType t of
              Arrow -> makeColor 0.2 0.4 1 1
              Cannon -> makeColor 1 0.5 0 1
              Ice -> makeColor 0.4 0.8 1 1
              Lightning -> makeColor 0.8 0.2 1 1
      size = 18 + fromIntegral (towerLevel t - 1) * 3
      
      -- Show range if hovered
      showRange = case hoveredCell state of
        Just cell -> worldToGrid (x, y) == cell
        Nothing -> False
      
      rangeCircle = if showRange
        then translate x y (color (makeColor 1 1 1 0.15) (ThickCircle (towerRange t) 2))
        else blank
      
      -- Level indicators
      levelStars = pictures [translate (x + dx) (y + 25) (color yellow (circleSolid 2)) 
                            | (dx, lvl) <- zip [-8, 0, 8] [1..], lvl <= towerLevel t]
      
      -- Tower icon based on type
      towerShape = case towerType t of
        Arrow -> pictures
          [ color col (circleSolid size)
          , color white (Polygon [(0, 8), (-5, -5), (5, -5)])
          ]
        Cannon -> pictures
          [ color col (rectangleSolid (size*1.2) (size*1.2))
          , color (dark col) (rectangleSolid (size*0.8) 5)
          , translate (size*0.6) 0 (color (dark col) (circleSolid 6))
          ]
        Ice -> pictures
          [ color col (Polygon [(0, size), (size*0.8, 0), (0, -size), (-size*0.8, 0)])
          , color white (circleSolid (size * 0.4))
          ]
        Lightning -> pictures
          [ color col (ThickCircle size 4)
          , color yellow (Polygon [(0, 10), (-4, 0), (0, -2), (4, -10)])
          ]
      
  in pictures [rangeCircle, translate x y towerShape, levelStars]
...
drawParticle p =
  let (x, y) = partPos p
      alpha = partLife p
      col = partColor p
  in translate x y (color col (circleSolid (partSize p)))
...
drawUI state =
  let panelBg = translate (-420) 280 (color (makeColor 0.1 0.1 0.15 0.9) (rectangleSolid 140 120))
      ...
      helpText = translate (-480) 115 (scale 0.08 0.08 (color (greyN 0.7) (text "[U] Upgrade tower")))
